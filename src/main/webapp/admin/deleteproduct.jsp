...
<c:import url="../assets/pattern-jsp/head.jsp">
	<c:param name="title" value="Delete product page" />
</c:import>
		...
		<table border="1">
			...
			<c:forEach var="s" items="${products}">
				<tr>
					...
					<td>${s.price}</td>
					<td>${s.number}</td>
					<td><a href="deleteproduct?id=${s.id}"
						onclick="return confirm('Are you sure ?');">Delete</a></td>
					<td><a href="${contextRoot}/admin/update?id=${s.id}">Update</a></td>
					<td><a href="${contextRoot}/admin/updatenumber?id=${s.id}">Update
							Number</a></td>
				</tr>
			</c:forEach>
		</table>
